...
const serwist = new Serwist({
  ...
  runtimeCaching: [
    ...
    // Corpus RAG y binario WASM del motor CPU: son parte de la app (no
    // pesos de modelo, esos ya los cachean WebLLM/wllama por su cuenta),
    // pero con la estrategia por defecto (NetworkOnly de defaultCache)
    // fallan si el usuario está realmente sin internet. CacheFirst los
    // deja disponibles offline después de la primera carga.
    {
      matcher: ({ sameOrigin, url: { pathname } }) =>
        sameOrigin &&
        (pathname === "/chunks.json" ||
          pathname.startsWith("/rag/") ||
          pathname.startsWith("/wllama/")),
      handler: new CacheFirst({
        cacheName: "EibRecursosLocales",
        plugins: [
          new ExpirationPlugin({
            maxEntries: 20,
