...
// Barra de progreso para la descarga/carga del modelo (primera vez que se
// usa, o cuando se cambia de modelo). Reemplaza el texto crudo que reporta
// WebLLM ("Fetching param cache[44/88]: ... 50% completed, 182 secs
// elapsed.") por una barra visual con porcentaje y tiempo transcurrido.
function ModelLoadingBar(props: { text: string; model?: string }) {
  const progreso = parseModelLoadProgress(props.text);
  if (!progreso) {
    return <>{props.text}</>;
  }

  return (
    <div className={styles["model-loading-bar"]}>
      {props.model && (
        <div className={styles["model-loading-bar-model"]}>
          {props.model}
        </div>
      )}
      <div className={styles["model-loading-bar-header"]}>
        <span>
          {progreso.step} {progreso.detail}
        </span>
        <span>{progreso.percent}%</span>
      </div>
      <div className={styles["model-loading-bar-track"]}>
        <div
          className={styles["model-loading-bar-fill"]}
          style={{ width: `${Math.min(Math.max(progreso.percent, 3), 100)}%` }}
        />
      </div>
      {typeof progreso.secsElapsed === "number" && (
        <div className={styles["model-loading-bar-time"]}>
          {progreso.secsElapsed}s transcurridos · se descarga una sola vez,
...
function _Chat() {
  ...
  const wllama = useContext(WllamaContext)!;

  const llm =
    config.modelClientType === ModelClient.MLCLLM_API
      ? mlcllm
      : config.modelClientType === ModelClient.WLLAMA_CPU
        ? wllama
        : webllm;
        ...
        {messages.map((message, i) => {
          ...
          const progresoCarga =
            !isUser && message.streaming
              ? parseModelLoadProgress(getMessageTextContent(message))
              : null;
