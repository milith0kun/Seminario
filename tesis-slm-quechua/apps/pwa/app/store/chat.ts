...
import { ModelConfig, Model, useAppConfig, ConfigType, ModelClient } from "./config";
...
import { detectLanguage } from "../rag/enrutadorLenguaje";
  ...
  (set, _get) => {
    ...
    const methods = {
      ...
      async onUserInput(
        ...
        // Enrutamiento de idioma EIB: se agrega como instrucción SOLO al
        // mensaje que se envía al modelo (contenidoParaEnviar), nunca al
        // texto que se guarda/muestra en el chat. El RAG (recuperación de
        // contexto) se resuelve aparte, dentro del cliente del modelo, así
        // que tampoco se antepone aquí — la burbuja del usuario muestra
        // siempre su pregunta tal cual la escribió.
        let contenidoParaEnviar = userContent;
        try {
          const idioma = detectLanguage(userContent);
          if (idioma === "qu_collao") {
            contenidoParaEnviar +=
              "\n\n[Instrucción de idioma: prioriza Quechua Collao en la respuesta.]";
          } else if (idioma === "mixed_es_qu") {
            contenidoParaEnviar +=
              "\n\n[Instrucción de idioma: puedes combinar español y Quechua Collao con claridad.]";
          }
        } catch (err) {
          console.warn("[Idioma] Falló la detección:", err);
        }
        ...
        // Mensaje que efectivamente se envía al modelo: igual al mostrado,
        // salvo por la instrucción de idioma (invisible en la burbuja).
        const mensajeParaEnviar: ChatMessage =
          typeof mContent === "string" && contenidoParaEnviar !== mContent
            ? { ...userMessage, content: contenidoParaEnviar }
            : userMessage;

        // get recent messages
        const recentMessages = get().getMessagesWithMemory();
        const sendMessages = recentMessages.concat(mensajeParaEnviar);
