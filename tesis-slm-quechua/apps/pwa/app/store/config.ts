...
// 3B: es el "modelo local por defecto" que define la tesis
// (documentacion/decisiones/0001-alcance-y-hardware.md), no un tamaño
// elegido por conveniencia.
// q4f32_1 (no q4f16_1): no todas las GPUs/navegadores soportan la
...
export const DEFAULT_CONFIG: ConfigType = {
  ...
  hideBuiltinTemplates: true, // no aplican al asistente EIB de propósito único
  ...
  (set, get) => ({
    ...
    setModels(models: ModelRecord[]) {
      if (models.some((m) => m.name === get().modelConfig.model)) {
        ...
        // El modelo guardado en este dispositivo (de una sesión anterior)
        // ya no está en el catálogo actual (ej. se recortó a solo Qwen2.5).
        // Se cambia al primero disponible, mostrando aviso para que no
        // pase inadvertido qué modelo se va a descargar.
        const modeloAnterior = get().modelConfig.model;
        if (typeof window !== "undefined") {
          showToast(
            `El modelo guardado "${modeloAnterior}" ya no está disponible. Se seleccionó "${models[0].name}".`,
          );
        }
        set((state) => ({
          ...state,
